...
if platform.mac_ver()[0] != '': # if we're on a mac
    if int(platform.mac_ver()[0].split('.')[1]) >= 7: # if the OS X version number is greater than or equal to 7
        extra_link_args.extend(['-L/Developer/SDKs/MacOSX10.6.sdk/usr/lib/'])
...
XTC = Extension('msmbuilder/libxdrfile',
                  ...
                  extra_compile_args=[],
                  extra_link_args=extra_link_args,# + ["-shared"],
...
LPRMSD = Extension('msmbuilder/_lprmsd',
                   ...
                   extra_link_args=extra_link_args + ['/opt/intel/Compiler/11.1/072/mkl/lib/em64t/libmkl_solver_lp64_sequential.a',
