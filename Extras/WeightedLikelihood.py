...
def GetAllLikelihoods(K,WhichTimes,Ckij):
    ...
    for k,Time in enumerate(WhichTimes):
        ...
        flist.append(np.sum(Ckij[Time]*np.log(T)))
    flist=np.array(flist)
    return flist

def WeightedLikelihood(K,WhichTimes,Ckij,Weights=None):
    """Calculate the total likelihood of a rate matrix given assignment data.
    ...
    for k,Time in enumerate(WhichTimes):
        T=scipy.linalg.matfuncs.expm(K*Time)
        #f+=np.sum(Ckij[Time]*np.log(T))*Weights[k]#KAB changed FEB
        f+=np.sum(Ckij[k]*np.log(T))*Weights[k]
    return f
...
def OptimizeRates(K0,Pi,WhichTimes,Ckij,Weights):
    ...
    def obj(X):
        ...
        f=WeightedLikelihood(K,WhichTimes,Ckij,Weights=Weights)
        return -1*f
...
def OptimizeSingleRates(K0,Pi,C):
    """Maximize the likelihood of a rate matrix given assignment data.
