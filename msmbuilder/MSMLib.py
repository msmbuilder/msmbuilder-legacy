...
def EstimateRateMatrix(tCount, Assignments):
    ...
    current  = K.diagonal()
    K = K - scipy.sparse.dia_matrix( (current, 0), C.shape).tocsr()
    row_sums = np.asarray( K.sum(axis=1) ).flatten() 
    K = K - scipy.sparse.dia_matrix( (row_sums, 0), C.shape).tocsr()
    assert K.shape == tCount.shape
    assert np.abs(K.sum()) < 10**-3
...
def PCCA(T,NumMacro,NumericalTolerance=1E-5):
    """Create a lumped model using the PCCA algorithm.  

    Inputs:
    T -- A transition matrix.  
    NumMacro -- The desired number of states.

    Optional Inputs:
    NumericalTolerance=1E-5 : specifies the numerical cutoff to use when splitting states based on sign.
    ...
    NumMicro=T.shape[0]
    Lam,EV=GetEigenvectors_Right(T,NumMacro)

    #Remove the stationary eigenvalue and eigenvector.
    Lam=Lam[1:]
    EV=EV[:,1:]

    mapMicroToMacro=np.zeros(NumMicro,'int')
    ...
    for i in range(NumMacro-1):#Thus, if we want 2 states, we split once.
        v=EV[:,i]
        AllSpreads=np.array([spread(v[mapMicroToMacro==k]) for k in range(i+1)])
        StateToBeSplit=np.argmax(AllSpreads)
        mapMicroToMacro[(mapMicroToMacro==StateToBeSplit)&(v>=NumericalTolerance)]=i+1
...
class PCCAPlusSolver():
    ...
    def PCCA_Simplex(self, nEigPerron, doMinimization=True):
        ...
        eigVals,eigVecsLeft=GetEigenvectors(self.T,nEigPerron+1)
        eigVals,eigVecs=GetEigenvectors_Right(self.T,nEigPerron+1)
