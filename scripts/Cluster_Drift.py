...
from Emsmbuilder.metrics import RMSD, DriftMetric
from Emsmbuilder import Serializer

def main():
    print """
    \nClusters data based on RMSD and Drift. Implements k-centers algorithm, followed by
    ...
    arglist=["projectfn", "clusters", "stride", "atomindices",
             "globalkmediods", "localkmediods", "rmsdcutoff","outdir","timecutoff"]
    ...
    trajs = [proj.LoadTraj(i)[::stride] for i in range(proj['NumTrajs'])]

    # cluster
    rmsd_metric = RMSD(atom_inds)
    drift_metric = DriftMetric( rmsd_metric, int(time_cutoff/stride) )
    clusterer = HybridKMedoids(drift_metric, trajs, k, rmsd_cutoff,
